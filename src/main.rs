...
fn export_nix_index_to_sqlite_with_transaction<F>(db_path: &str, dump_func: F)
where
    F: Fn(&mut dyn Write, Reader),
{
    ...
    dump_func(&mut handle, reader);

    writeln!(handle, "COMMIT;").unwrap();
}

fn dump_sqlite_for_fulltext_search(handle: &mut dyn Write, reader: Reader) {
    writeln!(
        handle,
...
fn dump_sqlite_normalized(handle: &mut dyn Write, reader: Reader) {
    writeln!(
        handle,
...
fn dump_sqlite_pkgconfig_libs(handle: &mut dyn Write, reader: Reader) {
    writeln!(
        handle,
        "CREATE TABLE packages (id INTEGER PRIMARY KEY, store_path TEXT UNIQUE);"
    )
